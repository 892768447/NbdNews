 ...
 * {@link android.support.v4.app.Fragment} call
...
public class SmartTabLayout extends HorizontalScrollView {
	...
	public SmartTabLayout(Context context, AttributeSet attrs, int defStyle) {
		...
		TypedArray a = context.obtainStyledAttributes(attrs,
				R.styleable.stl_SmartTabLayout, defStyle, 0);
		tabBackgroundResId = a.getResourceId(
				R.styleable.stl_SmartTabLayout_stl_defaultTabBackground,
				tabBackgroundResId);
		textAllCaps = a.getBoolean(
				R.styleable.stl_SmartTabLayout_stl_defaultTabTextAllCaps,
				textAllCaps);
		textColors = a
				.getColorStateList(R.styleable.stl_SmartTabLayout_stl_defaultTabTextColor);
		textSize = a
				.getDimension(
						R.styleable.stl_SmartTabLayout_stl_defaultTabTextSize,
						textSize);
		...
		textMinWidth = a.getDimensionPixelSize(
				R.styleable.stl_SmartTabLayout_stl_defaultTabTextMinWidth,
				textMinWidth);
		customTabLayoutId = a.getResourceId(
				R.styleable.stl_SmartTabLayout_stl_customTabTextLayoutId,
				customTabLayoutId);
		customTabTextViewId = a.getResourceId(
				R.styleable.stl_SmartTabLayout_stl_customTabTextViewId,
				customTabTextViewId);
		distributeEvenly = a.getBoolean(
				R.styleable.stl_SmartTabLayout_stl_distributeEvenly,
				distributeEvenly);
		a.recycle();
		...
		this.tabViewTextColors = (textColors != null) ? textColors
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		if (tabStrip.isIndicatorAlwaysInCenter() && getChildCount() > 0) {
			...
			int left = (w - Utils.getMeasuredWidth(firstTab)) / 2
					- Utils.getMarginStart(firstTab);
			int right = (w - Utils.getMeasuredWidth(lastTab)) / 2
	 ...
	 * prior to calling {@link #setViewPager(android.support.v4.view.ViewPager)}
	...
	/**
	 * Sets the colors used for styling the tab text. This will need to be
	 * called prior to calling
	 * {@link #setViewPager(android.support.v4.view.ViewPager)} otherwise it
	 * will not get set
	 * 
	 * @param colors
	...
	public void setCustomTabView(int layoutResId, int textViewId) {
		tabProvider = new SimpleTabProvider(getContext(), layoutResId,
				textViewId);
	}
	...
	protected TextView createDefaultTabView(CharSequence title) {
		...
		textView.setPadding(tabViewTextHorizontalPadding, 0,
	...
	private void populateTabStrip() {
		...
		for (int i = 0; i < adapter.getCount(); i++) {
			...
			final View tabView = (tabProvider == null) ? createDefaultTabView(adapter
					.getPageTitle(i)) : tabProvider.createTabView(tabStrip, i,
					adapter);
	...
	private void scrollToTab(int tabIndex, int positionOffset) {
		...
		if (selectedTab != null) {
			int targetScrollX = Utils.getLeft(selectedTab)
					- Utils.getMarginStart(selectedTab) + positionOffset;
	...
	public interface TabProvider {
		 ...
		 */
		View createTabView(ViewGroup container, int position,
	...
	private static class SimpleTabProvider implements TabProvider {
		...
		private SimpleTabProvider(Context context, int layoutResId,
				int textViewId) {
		...
		public View createTabView(ViewGroup container, int position,
			...
			if (tabViewTextViewId != NO_ID && tabView != null) {
				tabTitleView = (TextView) tabView
						.findViewById(tabViewTextViewId);
			}
	...
	private class InternalViewPagerListener implements
		...
		public void onPageScrolled(int position, float positionOffset,
			...
			int widthPlusMargin = Utils.getWidth(selectedTab)
					+ Utils.getMarginHorizontally(selectedTab);
			...
			if (0f < positionOffset && positionOffset < 1f
				...
				int current = Utils.getWidth(currentTab) / 2
						+ Utils.getMarginEnd(currentTab);
				int next = Utils.getWidth(nextTab) / 2
						+ Utils.getMarginStart(nextTab);
			...
			if (viewPagerPageChangeListener != null) {
				viewPagerPageChangeListener.onPageScrolled(position,
